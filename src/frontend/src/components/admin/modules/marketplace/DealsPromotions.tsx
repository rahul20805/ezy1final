      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-xl sm:text-2xl font-display font-bold text-foreground flex items-center gap-2">
            <Zap className="w-5 h-5 text-amber-500 fill-amber-500" /> Flash Deals & Promotional Campaigns
          </h1>
          <p className="text-xs sm:text-sm text-muted-foreground mt-1">
            Manage limited-time flash sales, countdown deals, and homepage promotional spotlights.
          </p>
        </div>
        ...
        {flashDeals.map((deal) => (
          <Card
            ...
            className={`rounded-3xl border transition-all p-5 space-y-3 ${
              deal.active ? "border-amber-500/40 bg-card" : "border-border/60 bg-muted/20 opacity-70"
            }`}
          >
            <div className="flex items-start justify-between gap-2">
              <Badge className="bg-amber-500/10 text-amber-600 border-amber-500/30 text-[10px] font-bold">
                <Flame className="w-3 h-3 mr-1 fill-amber-500" /> {deal.discount}
              </Badge>
              <Badge variant="outline" className="text-[10px] font-mono">
                <Clock className="w-3 h-3 mr-1 text-muted-foreground" /> {deal.endsIn}
              </Badge>
            </div>

            <h3 className="font-display font-bold text-sm text-foreground">{deal.title}</h3>
            <p className="text-xs text-muted-foreground">{deal.category}</p>

            <div className="flex items-center justify-between pt-3 border-t border-border/60">
              <span className={`text-xs font-semibold ${deal.active ? "text-emerald-600" : "text-muted-foreground"}`}>
