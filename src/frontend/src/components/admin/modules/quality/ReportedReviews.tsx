...
export function ReportedReviews() {
  ...
  // Flagged reviews
  const reportedReviews = store.reviews.filter((r) => r.isReported || r.rating <= 2);
  ...
  return (
    <div className="space-y-6">
      <DataTable<StoredReview>
        ...
        pageSize={6}
        renderItem={(rev) => (
          <Card key={rev.id} className="rounded-3xl border-rose-500/30 bg-rose-500/5 p-5 shadow-xs space-y-3">
            <div className="flex items-start justify-between gap-3">
              <div>
                <div className="flex items-center gap-2">
                  <span className="font-display font-bold text-sm text-foreground">{rev.author}</span>
                  <Badge variant="outline" className="text-[10px] uppercase font-bold text-rose-600 border-rose-300">
                    Flagged for Review
                  </Badge>
                </div>
                <p className="text-xs text-muted-foreground mt-0.5">
                  Target: <span className="font-semibold text-foreground">{rev.targetName}</span> ({rev.targetType}) • {rev.date}
                </p>
              </div>
            ...
            <div className="flex items-center justify-between pt-2 border-t border-border/60">
              <span className="text-[11px] text-muted-foreground">Report Reason: Inappropriate content or dispute</span>
              ...
              <div className="flex items-center gap-2">
                ...
                >
                  <CheckCircle2 className="w-3.5 h-3.5 mr-1 text-emerald-500" /> Dismiss & Keep
                </Button>
                <Button
                  size="sm"
