...
import { type StoredSupportTicket, useStoreData } from "../../../../lib/storeData";
import { DataTable } from "../../../owner/DataTable";

export function SupportTickets() {
  const store = useStoreData();

  const [selectedTicket, setSelectedTicket] = useState<StoredSupportTicket | null>(null);
  ...
  const handleSendReply = (e: React.FormEvent) => {
    ...
    setReplyText("");
    setSelectedTicket(store.supportTickets.find((t) => t.id === selectedTicket.id) || null);
  };

  const handleStatusChange = (ticketId: number, status: StoredSupportTicket["status"]) => {
    store.updateSupportTicketStatus(ticketId, status);
    toast.success(`Ticket marked as: ${status}`);
    if (selectedTicket && selectedTicket.id === ticketId) {
      setSelectedTicket(store.supportTickets.find((t) => t.id === ticketId) || null);
    }
  };
          ...
          >
            <CardContent className="p-5 space-y-4">
              <div className="flex items-start justify-between gap-3">
                <div>
                  <div className="flex items-center gap-2">
                    <span className="font-mono font-bold text-sm text-foreground">{ticket.ticketNumber}</span>
                    <Badge className={`text-[9px] uppercase font-bold ${getPriorityColor(ticket.priority)}`}>
                      {ticket.priority}
                    </Badge>
                  </div>
                  <h4 className="font-display font-bold text-sm text-foreground mt-1">{ticket.subject}</h4>
                  <p className="text-xs text-muted-foreground">
                    {ticket.customerName} ({ticket.customerPhone}) • {ticket.createdAt}
                  </p>
                </div>
              ...
              <div className="p-3 rounded-2xl bg-muted/40 border border-border/60 text-xs space-y-1">
                <span className="text-[10px] text-muted-foreground font-semibold uppercase">Latest Message:</span>
              ...
              <div className="flex items-center justify-between pt-2 border-t border-border/60">
                <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
                  <span>Assigned:</span>
                  <span className="font-bold text-foreground">{ticket.assignedAdmin || "Support Queue"}</span>
                </div>
                ...
                >
                  <MessageSquare className="w-3.5 h-3.5" /> View & Reply ({ticket.messages.length})
                </Button>
              </div>
            </CardContent>
          </Card>
            ...
            <div className="space-y-4">
              <DialogHeader>
                <div className="flex items-center justify-between">
                  ...
                  <Badge className={`text-[10px] uppercase font-bold ${getPriorityColor(selectedTicket.priority)}`}>
              ...
              <div className="max-h-60 overflow-y-auto space-y-2 p-3 bg-muted/30 border border-border rounded-2xl text-xs">
                {selectedTicket.messages.map((m, i) => (
                  ...
                  >
                    <p>{m.text}</p>
                    <span className="text-[9px] opacity-75 block text-right mt-1 font-mono">{m.timestamp}</span>
                  </div>
                ))}
              </div>
              ...
              <form onSubmit={handleSendReply} className="space-y-2">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <Button type="submit" size="sm" className="rounded-xl text-xs gap-1.5 font-semibold">
