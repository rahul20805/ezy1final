...
export function OwnerSettingsSection() {
  ...
  const { currentPartner, partners, addPartner, updatePartner, deletePartner, resetPartnersToDefault } = usePartnerAuth();

  const isSuperOwner = currentPartner?.role === "super_owner";

  // Form State
  const [formData, setFormData] = useState<OwnerSettings>({ ...store.settings });
  ...
  const handleChange = <K extends keyof OwnerSettings>(key: K, value: OwnerSettings[K]) => {
  ...
  const handleSaveAllSettings = (e: React.FormEvent) => {
    ...
    toast.success("Owner Settings saved & immediately synchronized with live website!");
  };
  ...
  const handleCreatePartnerAccount = (e: React.FormEvent) => {
    e.preventDefault();
    if (!newPartnerId.trim() || !newPartnerPassword.trim() || !newPartnerBusiness.trim()) {
      toast.error("Partner ID, Password and Business Name are required.");
      return;
    }

    if (partners.some((p) => p.id.toLowerCase() === newPartnerId.trim().toLowerCase())) {
    ...
    addPartner({
      ...
      permissions: {
        ...
        canManageBookings: newPartnerCategory === "Workshops" || newPartnerCategory === "Healthcare",
    ...
    toast.success(`New Partner Account "${newPartnerId}" created with individual access!`);
    setNewPartnerId("");
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs sm:text-sm text-muted-foreground mt-1">
            Configure business identity, WhatsApp, delivery rules, UPI payments, SEO, store policies, and manage partner logins.
          </p>
        </div>
      ...
      <form onSubmit={handleSaveAllSettings}>
        <Tabs defaultValue="business" className="space-y-6">
          <TabsList className="bg-muted/80 p-1 rounded-2xl flex-wrap h-auto gap-1">
            <TabsTrigger value="business" className="rounded-xl text-xs gap-1.5">
              <Building2 className="w-3.5 h-3.5" /> Business Info
            </TabsTrigger>
            <TabsTrigger value="delivery" className="rounded-xl text-xs gap-1.5">
              <Truck className="w-3.5 h-3.5" /> Delivery & Hours
            </TabsTrigger>
            <TabsTrigger value="payments" className="rounded-xl text-xs gap-1.5">
            ...
            <TabsTrigger value="policies" className="rounded-xl text-xs gap-1.5">
              <Shield className="w-3.5 h-3.5" /> Policies
            </TabsTrigger>
            {isSuperOwner && (
              <TabsTrigger value="partners" className="rounded-xl text-xs gap-1.5">
                <Lock className="w-3.5 h-3.5" /> Partner Logins ({partners.length})
              </TabsTrigger>
            )}
            <TabsTrigger value="database" className="rounded-xl text-xs gap-1.5">
          ...
          <TabsContent value="business">
            <Card className="rounded-3xl border-border bg-card shadow-xs">
              <CardHeader>
                <CardTitle className="text-base font-display font-bold">Business Identification & Contact</CardTitle>
                <CardDescription className="text-xs">
                  This information appears across the website header, footer, invoices and WhatsApp click-to-chat.
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  <div className="space-y-1.5">
                    <Label className="text-xs font-semibold">Official WhatsApp Number (Click-to-Chat) *</Label>
                    <Input
                  ...
                  <div className="space-y-1.5">
                    <Label className="text-xs font-semibold">Customer Support Phone</Label>
                    <Input
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  ...
                  <div className="space-y-1.5">
                    <Label className="text-xs font-semibold">GST / Tax Registration Number</Label>
                    <Input
          ...
          <TabsContent value="delivery">
            <Card className="rounded-3xl border-border bg-card shadow-xs">
              <CardHeader>
                <CardTitle className="text-base font-display font-bold">Operational Hours & Delivery Rules</CardTitle>
                <CardDescription className="text-xs">
                  Set delivery fees, free delivery limits, radius and business working hours.
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="space-y-1.5">
                    <Label className="text-xs font-semibold">Operational Hours Text</Label>
                    <Input
                  ...
                  <div className="flex items-center justify-between p-3.5 rounded-2xl bg-muted/40 border border-border">
                    <div>
                      <Label className="text-xs font-semibold">Open For Orders Today</Label>
                      <p className="text-[11px] text-muted-foreground">Accept live orders on site</p>
                    </div>
                    <Switch
                      checked={formData.isOpenToday}
                      onCheckedChange={(val) => handleChange("isOpenToday", val)}
                    />
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
                  ...
                  <div className="space-y-1.5">
                    <Label className="text-xs font-semibold">Service Delivery Radius (km)</Label>
                    <Input
                      type="number"
          ...
          <TabsContent value="payments">
            <Card className="rounded-3xl border-border bg-card shadow-xs">
              <CardHeader>
                <CardTitle className="text-base font-display font-bold">Payment Methods & UPI Gateway</CardTitle>
                <CardDescription className="text-xs">
                  Configure UPI IDs for QR code / Intent payments and toggle supported checkout options.
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                <div className="space-y-1.5">
                  <Label className="text-xs font-semibold">Business UPI ID (GPay / PhonePe / Paytm) *</Label>
                  <Input
                  ...
                  />
                  <p className="text-[11px] text-muted-foreground">Used to receive direct UPI payments from customers at checkout.</p>
                </div>

                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-2">
                  <div className="flex items-center justify-between p-3.5 rounded-2xl bg-muted/40 border border-border">
                    <div>
                      <Label className="text-xs font-semibold">Enable Instant UPI</Label>
                      <p className="text-[11px] text-muted-foreground">GPay, PhonePe, Paytm, BHIM</p>
                    </div>
                    <Switch checked={formData.enableUpi} onCheckedChange={(val) => handleChange("enableUpi", val)} />
                  </div>

                  <div className="flex items-center justify-between p-3.5 rounded-2xl bg-muted/40 border border-border">
                    <div>
                      <Label className="text-xs font-semibold">Enable Cash on Delivery (COD)</Label>
                      <p className="text-[11px] text-muted-foreground">Collect cash/UPI upon delivery</p>
                    </div>
                    <Switch checked={formData.enableCod} onCheckedChange={(val) => handleChange("enableCod", val)} />
                  </div>

                  <div className="flex items-center justify-between p-3.5 rounded-2xl bg-muted/40 border border-border">
                    <div>
                      <Label className="text-xs font-semibold">Enable In-App Wallet</Label>
                      <p className="text-[11px] text-muted-foreground">Allow instant checkout using Ezy1 balance</p>
                    </div>
                    <Switch checked={formData.enableWallet} onCheckedChange={(val) => handleChange("enableWallet", val)} />
                  </div>

                  <div className="flex items-center justify-between p-3.5 rounded-2xl bg-muted/40 border border-border">
                    <div>
                      <Label className="text-xs font-semibold">Enable Online Card Payments</Label>
                      <p className="text-[11px] text-muted-foreground">Debit & Credit card gateway</p>
                    </div>
                    <Switch checked={formData.enableOnlineCards} onCheckedChange={(val) => handleChange("enableOnlineCards", val)} />
                  </div>
                </div>
              </CardContent>
            </Card>
          ...
          <TabsContent value="seo">
            <Card className="rounded-3xl border-border bg-card shadow-xs">
              <CardHeader>
                <CardTitle className="text-base font-display font-bold">SEO Metadata & Social Handles</CardTitle>
                <CardDescription className="text-xs">
                  Optimize search engine indexing and link your official social channels.
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                <div className="space-y-1.5">
          ...
          <TabsContent value="policies">
            <Card className="rounded-3xl border-border bg-card shadow-xs">
              <CardHeader>
                <CardTitle className="text-base font-display font-bold">Store Policies & Legal Terms</CardTitle>
                <CardDescription className="text-xs">
                  Customize return windows, cancellation charges, terms of service and privacy guarantees.
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="space-y-1.5">
                    <Label className="text-xs font-semibold">Return & Replacement Policy</Label>
                    <Textarea
                      rows={3}
                  ...
                  <div className="space-y-1.5">
                    <Label className="text-xs font-semibold">Order Cancellation Policy</Label>
                    <Textarea
                      rows={3}
          ...
          {isSuperOwner && (
            <TabsContent value="partners" className="space-y-6">
              ...
              <Card className="rounded-3xl border-border bg-card shadow-xs">
                <CardHeader>
                  <CardTitle className="text-base font-display font-bold">Create New Partner Login & Access</CardTitle>
                  <CardDescription className="text-xs">
                    Issue individual credentials to store owners, healthcare providers, drivers and service specialists.
                  </CardDescription>
                </CardHeader>
                <CardContent>
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                    <div className="space-y-1">
                      <Label className="text-xs font-semibold">Unique Login / Admin ID *</Label>
                      <Input
                        required
              ...
              <div className="space-y-3">
                <h3 className="font-display font-bold text-base">Active Partner Credentials Directory</h3>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {partners.map((partner) => (
                    <Card key={partner.id} className="rounded-2xl border-border bg-card p-4 shadow-xs">
                      <div className="flex items-start justify-between gap-2">
                        <div>
                          <div className="flex items-center gap-2">
                            <h4 className="font-display font-bold text-sm text-foreground">{partner.businessName}</h4>
                            <Badge variant="outline" className="text-[10px] uppercase font-bold">
                              {partner.category}
                            </Badge>
                          </div>
                          <p className="text-xs text-muted-foreground mt-0.5">{partner.ownerName}</p>
                        </div>
                        <Badge
                          className={`text-[10px] uppercase font-bold ${
                            partner.role === "super_owner" ? "bg-primary text-primary-foreground" : "bg-muted text-foreground"
                          }`}
                        >
          ...
          <TabsContent value="database">
            <Card className="rounded-3xl border-border bg-card shadow-xs">
              <CardHeader>
                <CardTitle className="text-base font-display font-bold">Database Management & Reset Tools</CardTitle>
                <CardDescription className="text-xs">
                  Export complete data backup to JSON, restore from past backup file, or reset to original clean seed data.
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  <div className="p-4 rounded-2xl border border-border bg-muted/20 space-y-2">
                    <h4 className="font-semibold text-xs text-foreground">Export Backup (JSON)</h4>
                    <p className="text-[11px] text-muted-foreground">
                      Downloads all products, orders, services, customers and settings to a JSON file.
                    </p>
                    <Button
                      type="button"
                      variant="outline"
                  ...
                  <div className="p-4 rounded-2xl border border-border bg-muted/20 space-y-2">
                    <h4 className="font-semibold text-xs text-foreground">Restore From Backup</h4>
                    <p className="text-[11px] text-muted-foreground">
                      Paste a previously exported JSON backup to instantly restore full database state.
                    </p>
                    <Button
                      type="button"
                      variant="outline"
                  ...
                  <div className="p-4 rounded-2xl border border-destructive/30 bg-destructive/5 space-y-2">
                    <h4 className="font-semibold text-xs text-destructive">Reset to Clean Seeds</h4>
                    <p className="text-[11px] text-muted-foreground">
                      Resets all products, categories, orders and settings back to original clean demo seeds.
                    </p>
                    <Button
                      type="button"
                      variant="destructive"
      ...
      <ConfirmModal
        ...
        onConfirm={() => {
          ...
          setFormData({ ...store.settings });
          toast.success("Database and Partner credentials reset to default seeds!");
      ...
      {isImportModalOpen && (
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="w-full max-w-lg bg-card border border-border rounded-3xl p-6 shadow-2xl space-y-4">
            <h3 className="font-display font-bold text-lg">Restore Database from JSON</h3>
            ...
            <div className="flex items-center justify-end gap-2">
              <Button variant="outline" size="sm" onClick={() => setIsImportModalOpen(false)} className="rounded-xl">
                Cancel
              </Button>
              <Button size="sm" onClick={handleImportJson} className="rounded-xl bg-primary text-primary-foreground font-semibold">
                Import & Restore
              </Button>
            </div>
          </div>
