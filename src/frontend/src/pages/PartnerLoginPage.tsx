...
export default function PartnerLoginPage() {
  ...
  const handleLoginSubmit = (e: React.FormEvent) => {
    ...
    setIsLoading(true);
    setTimeout(() => {
      const res = login(partnerId, password);
      setIsLoading(false);

      if (res.success) {
        toast.success(`Welcome to ${store.settings.brandName} Owner & Partner Portal!`);
        navigate({ to: "/owner" });
      } else {
        ...
        <div className="w-full max-w-4xl space-y-6">
          ...
          <div className="text-center space-y-2">
            ...
            <h1 className="font-display font-black text-2xl sm:text-4xl text-foreground tracking-tight">
              Manage Your Business on <span className="text-primary">{store.settings.brandName}</span>
            </h1>
            <p className="text-xs sm:text-sm text-muted-foreground max-w-lg mx-auto">
              Enter your unique Partner Admin ID and Password to manage inventory, bookings, live orders, website content, and owner settings.
            </p>
          </div>
            ...
            <Card className="lg:col-span-7 rounded-3xl border-border bg-card shadow-lg">
              <CardHeader className="pb-4">
                <CardTitle className="text-lg font-display font-bold flex items-center gap-2">
                  <ShieldCheck className="w-5 h-5 text-primary" /> Partner Admin Login
                </CardTitle>
                <CardDescription className="text-xs">
                  Each partner has individual credentials and role-specific access.
                </CardDescription>
              </CardHeader>
              ...
              <CardContent>
                <form onSubmit={handleLoginSubmit} className="space-y-4">
                  <div className="space-y-1.5">
                    <Label className="text-xs font-semibold">Partner / Admin ID *</Label>
                  ...
                  <div className="space-y-1.5">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-[11px] text-muted-foreground">Default: admin123 / partner123</span>
                    </div>
                    <div className="relative">
                ...
                <div className="mt-6 pt-5 border-t border-border space-y-2.5">
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-bold text-foreground flex items-center gap-1.5">
                      <Sparkles className="w-3.5 h-3.5 text-primary" /> 1-Click Quick Demo Logins:
                    </span>
                    <span className="text-[10px] text-muted-foreground">Click any to auto-fill</span>
                  </div>
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                    ...
                    <button
                      type="button"
                      onClick={() => fillDemoAccount("sharma_grocery", "partner123")}
                      className={`p-2 rounded-xl text-left border text-xs transition-all ${
                    ...
                    >
                      ...
                      <p className="text-[10px] text-muted-foreground font-mono">sharma_grocery</p>
                    </button>

                    <button
                      type="button"
                      onClick={() => fillDemoAccount("nair_pharma", "partner123")}
                      className={`p-2 rounded-xl text-left border text-xs transition-all ${
                    ...
                    >
                      ...
                      <p className="text-[10px] text-muted-foreground font-mono">nair_pharma</p>
                    </button>

                    <button
                      type="button"
                      onClick={() => fillDemoAccount("suresh_services", "partner123")}
                      className={`p-2 rounded-xl text-left border text-xs transition-all ${
                    ...
                    >
                      ...
                      <p className="text-[10px] text-muted-foreground font-mono">suresh_services</p>
                    </button>

                    <button
                      type="button"
                      onClick={() => fillDemoAccount("rajesh_transport", "partner123")}
                      className={`p-2 rounded-xl text-left border text-xs transition-all ${
                    ...
                    >
                      ...
                      <p className="text-[10px] text-muted-foreground font-mono">rajesh_transport</p>
                    </button>
                  </div>
                </div>
              </CardContent>
                ...
                <ul className="space-y-3">
                  {perks.map((perk, i) => (
                    <li key={i} className="flex items-start gap-2.5 text-xs text-muted-foreground">
                      ...
                      <span className="text-foreground font-medium leading-relaxed">{perk}</span>
                    </li>
                  ))}
                </ul>

                <div className="p-4 rounded-2xl bg-primary/5 border border-primary/20 text-xs space-y-1">
                  <p className="font-bold text-primary">Need a New Partner Account?</p>
                  <p className="text-muted-foreground text-[11px]">
                    Log in as Master Admin (<span className="font-mono font-bold">admin / admin123</span>) to generate custom login IDs and assign permissions in Owner Settings.
                  </p>
                </div>
              </Card>
            </div>
